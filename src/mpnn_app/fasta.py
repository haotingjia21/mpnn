...
def load_original_and_designed_from_out(
    out_dir: Path, chains: List[str]
) -> Tuple[Dict[str, str], List[DesignedSequence]]:
    """Read ProteinMPNN outputs and treat the FIRST FASTA record as the original.
    ...
    chain_list = [c for c in chains if c] or ["A"]
    ...
    for fp in fasta_files:
        ...
        for rec in records[1:]:
            ...
            if len(chain_list) <= 1:
                designed.append(
                    DesignedSequence(chain=chain_list[0], rank=global_rank, sequence=seq, diff_positions=[])
                )
                global_rank += 1
                continue
