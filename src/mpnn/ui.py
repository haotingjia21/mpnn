# src/mpnn/ui.py  (replace your file with this)
...
# Dash is optional for headless/test environments.
try:
...
def _highlight_by_comparison(seq: str, original: str) -> html.Div:
    if html is None:  # pragma: no cover
        raise ModuleNotFoundError("dash is not installed")
    children: List[Union[str, html.Span]] = []
    L = max(len(seq), len(original))
    for i in range(L):
        ch = seq[i] if i < len(seq) else ""
        orig = original[i] if i < len(original) else ""
        if ch and (orig == "" or ch != orig):
...
def _render_results(data: Dict) -> html.Div:
    ...
    for chain in chain_order:
        ...
        for d in chain_designs:
            ...
            rows.append(_highlight_by_comparison(seq, orig_seq))
...
def create_dash_server():
            ...
            html.Div(
                [
                    ...
                    dcc.Input(
                        id="chains_text",
                        type="text",
                        placeholder='chains (optional, e.g. "A B" or "A,B"; empty = all chains)',
                        value="",
                        style={"width": "360px"},
                    ),
                    dcc.Dropdown(
    ...
    def _show_filename(filename: Optional[str]):
        if not filename:
            return ""
        ok = filename.lower().endswith((".pdb", ".cif", ".mmcif"))
        if not ok:
    ...
    def _on_design(_n_clicks, contents, filename, chains_text, model_name, nseq):
        if not contents or not filename:
            return html.Span("upload a structure file", style=S_ERR), ""
        if not filename.lower().endswith((".pdb", ".cif", ".mmcif")):
        ...
        payload: Dict = {"num_sequences": int(nseq) if nseq is not None else 5, "model_name": model_name}
        ...
        if r.ok:
            ...
            status = f"ok ({m.get('runtime_ms','?')} ms)"
